...
import random
import numpy as np
from modify_prediction import modify_prediction, validate_invnum
    ...
    try:
        ...
        if (filt.shape[0])>0:
            for index,row in filt.iterrows():
                ...
                print("int(row['right']):",(row['right']),"int(row['left'])",(row['left']),"float(row['bottom'])",float(row['bottom']),"float(row['top']))",float(row['top']))
                print( (a>=int(row['right'])) , (b<=int(row['left'])) , (c>=int(row['bottom'])),(d<=int(row['top'])) )
                if ((a>=float(row['right'])) | (b<=float(row['left'])) | (c>=float(row['bottom'])) | (d<=float(row['top']))):
                    ...
                    print("bbox matching but word shape not matching :",row["doc_id"])
