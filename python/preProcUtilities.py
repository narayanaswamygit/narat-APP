...
def upload_file_to_s3(file_name, bucket='invoice-digitisation', object_name=None):
    ...
    s3_client = boto3.client(   's3',
                                aws_access_key_id = base64_decode(cfg.getAWS_ACCESS_KEY_ID()),
                                aws_secret_access_key = base64_decode(cfg.getAWS_SECRET_ACCESS_KEY()),
...
def saveTOJson(fileName,myDict):
    try:
        with open(fileName, 'w') as outfile:
            json.dump(myDict, outfile)
        return True
    except:
        print("saving file exception",traceback.print_exc())
        return False
