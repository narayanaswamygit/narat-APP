    ...
    try:
        ...
        for idx,fld in enumerate(extractedFields):
            ...
            if fldId == "vendorGSTIN":
                v_gstin = fldVal
                ##  Added update non-sla vendor flag in case there is correction in vendor gstin 31 March 2023
                if "correctedValue" in fld.keys():
                    nonSLAVVendorFlag = add_non_sla_vendor_flag(vendorGSTIN=fldVal)
                    update_non_sla_vendor_flag(documentId=documentId,callback_url=callBackUrl,flag=nonSLAVVendorFlag)
                ##  Added update non-sla vendor flag in case there is correction in vendor gstin 31 March 2023
            #Do Mandatory check
...
def check_field_stp_score(documentId, callbackUrl):
    ...
    try:
        ...
        if docResult is not None:
            ...
            for docInfo_ in docInfo:
                #print(docInfo_["fieldId"])
                stp_score_field = ["invoiceDate","invoiceNumber","subTotal","totalAmount","CGSTAmount","SGSTAmount","IGSTAmount"]
...
def add_non_sla_vendor_flag(vendorGSTIN:str=None)->bool:
    try:
        print("vendorGSTIN to check :",vendorGSTIN)
        if vendorGSTIN:
            path = cfg.getNonSLAVendorFilePath()
            print("path :",path)
            df = pd.read_excel(os.path.join(path))
            print("non sla df shape :",df.shape)
            df = df[df["Non SLA Flag"]==1]
            GSTIN_LIST = list(df["Vendor GSTIN"])
            print("non sla GSTIN_LIST list :",GSTIN_LIST)
            if vendorGSTIN in GSTIN_LIST:
                return 1
        return 0
    except Exception as E:
        print(f"add_non_sla_vendor_flag exception : {E}")
        return 0

def get_vendorGSTIN_from_docApiInfoInp(docInfoAPI:dict=None)->str or None:
    try:
        if docInfoAPI:
            documentInfo = docInfoAPI.get("result").get("documentInfo")
            #print("documentInfo :",documentInfo)
            for f in documentInfo:
                if f.get("fieldId")=="vendorGSTIN":
                    if "correctedValue" in f.keys():
                        gstin = f.get("correctedValue") 
                    else :
                        gstin = f.get("fieldValue")
                    print("gstin :",gstin)
                    return gstin
        return
    except :
        print(f"get_vendorGSTIN_from_docApiInfoInp exception :{traceback.print_exc()}")
        return

def update_non_sla_vendor_flag(documentId:str,callback_url:str,flag:bool=0):
    try:
        data = {
                  ...
                  "request": {
                                ...
                                "nonSLAVendorFlag": flag
                              }
                }
        print(data)
        updateFlag = putil.updateDocumentApi_New(documentId=documentId,docInfo=data,callbackUrl=callback_url)
        print("updateFlag status :",updateFlag)
        return
    except:
        print(f"update_non_sla_vendor_flag:{traceback.print_exc()}")
        return
