...
def extraction(encryptedInput):
    ...
    def updateAuthInDb(auth_token,
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                ins_string += "(sub_id, call_id,auth_token,authenticated_time,"
                ins_string += " expiration_time) "
    ...
    def auth_subscriber(sub_id, mac_id):
        try:
            ...
            print("Auth_subscriber",
                  traceback.print_exc())
            return None

    def check_subscriber_limit(sub_id_, no_of_pages,no_of_docs=1):
        def check_validity(df):
            ...
            if df[df['date'] > pd.Timestamp.now()].shape[0] != 1:
                ...
                return False, 101
            try:
                if df["type"].unique()[0].lower() == "paiges":
                    ...
                    if df['page_diff'].iloc[0] <= 0:
                        ...
                        return False, 102
                    return True, 1
                elif df["type"].unique()[0].lower() == "tapp":
                    df["input_docs"] = no_of_docs
                    print(df["input_docs"], "docs")
                    # Authenticate docs
                    df['docs_diff'] = df['allotted'].sub(df['docs_requested'], axis=0)

                    df["docs_diff"] = df["docs_diff"].sub(df["input_docs"],
                                                        axis=0)
                    print(df["docs_diff"], "xxx")
                    if df['docs_diff'].iloc[0] < 0:
                        print("Request limit exhausted")
                        return False, 102
                    return True, 1
            except:
                print("Database Issues while fetching unique type values of view zuth_by_sub_id")
                return False, 100
        try:
    ...
    try:
        ...
        print("Time taken to decrypt message is:",time_1a - time_1)
        print("Time taken to auth_subscriber is:", time_2a - time_2)
        print("Time taken to check subsciber limit is:", time_3a - time_3)
        print("Time taken to generate auth token is:", time_4a - time_4)
        print("Time taken to triggerExtractionSubmit is:", time_5a - time_5)
        print("Time taken to update auth in db is:", time_6a - time_6)
        print("Total time taken is:", time.time()-time_total)
        ...
        print("Extraction Authentication:",
              traceback.print_exc())
        return returnFailure()
    finally:
