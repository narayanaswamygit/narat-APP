...
def extract_features(docInput,
    ...
    try:
        ...
        DF["page_num"] = pageNumber
        DF = DF.sort_values(["page_num",
                             "line_num",
                             "word_num"])
        DF.reset_index(inplace = True)
        blob_ftr_file_name = documentId + "_pageNo_" + str(pageNumber) + "_singleFtrs.csv"
        blob_ftr_file_path = os.path.join(ROOT_FOLDER,
                                          client_folder,
                                          blob_ftr_file_name)
        if os.path.exists("d:"):
            DF.to_csv("d:/singleFtrs_" + str(pageNumber) + ".csv",
                      index = False)
        DF.to_csv(blob_ftr_file_path)
