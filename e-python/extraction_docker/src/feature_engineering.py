...
# In[4]: Declare Functions to extract features for line items
# @util.timing
def check_hdr(DF,hdr_line):
    try:
        ...
        #Apr-27-2023 - Hari - Check wordshapes of header if it has more than 2 words with 'd's then they are not header
        word_shapes = DF.loc[(DF["tableLineNo"] == hdr_line)]["wordshape"].to_list()
        #Apr-27-2023 - Hari - Check wordshapes of header if it has more than 2 words with 'd's then they are not header
        # no_count = sum([1 if (util.isNumber(text) or util.isAmount(text))
        #                 else 0 for text,line_text in zip(texts,line_texts)])
        print("line texts",line_texts,texts)
        no_count = sum([1 if (util.isAmount(text))
                        else 0 for text,line_text in zip(texts,line_texts)])
        print("Count",no_count)
        #Apr-27-2023 - Hari - Check wordshapes of header if it has more than 2 words with 'd's then they are not header
        d_count = sum([1 if ('d' in text) else 0 for text in word_shapes])
        #Apr-27-2023 - Hari - Check wordshapes of header if it has more than 2 words with 'd's then they are not header

        #Apr-27-2023 - Hari - Check wordshapes of header if it has more than 2 words with 'd's then they are not header
        # if (no_count > 0):
        if (no_count > 0) or (d_count > 2):
        #Apr-27-2023 - Hari - Check wordshapes of header if it has more than 2 words with 'd's then they are not header
            return False
        else:
            return True
...
#         t = time.time()
#         max_lefts = []
#         #May 31 2023 - Take the dimensions of extreme left & right to
#         max_rights = []
#         #May 31 2023 - Take the dimensions of extreme left & right to
#         max_alls = []
...
#         #May 31 2023 - Take the dimensions of extreme left & right to
#         #check overlap condition for row breaks
...
#         max_left = min(max_lefts)
#         max_ind = max_lefts.index(max_left)
#         max_all = max_alls[max_ind]
#         max_right = max(max_rights)
#         row_number = 1
#         row_first = False
...
#             right = all_[1]
#             line_no = all_[2]
#             print("line rows assignment:",line_no,row_number)
...
def correctLineRows(df):
    ...
    ptn = 'page[ ]{0,1}[0-9]{1,3}[ ]{0,1}of[ ]{0,1}[0-9]{1,3}'
    ...
    try:
        ...
        for page in pages.groups:
            ...
            #Mar-19-2023 - Remove line rows after page number
            #Apr-25-2023 - Use line_row only
            # line_rows = df[(df["page_num"] == page) &
            #                (df["line_row_new"] > 0)]
            line_rows = df[(df["page_num"] == page) &
                           (df["line_row"] > 0)]
            #Apr-25-2023 - Use line_row only
            line_no_grps = line_rows.groupby(["tableLineNo"])
            brk = False
            for line_no in line_no_grps.groups:
                if brk:
                    break
                line_row_no = line_rows[line_rows["tableLineNo"] == line_no]
                # print("line_row_no",type(line_row_no))
                for line_index,line_row in line_row_no.iterrows():
                    # print("line_row",type(line_row),line_row)
                    if brk:
                        break
                    line_text = line_row["line_text"].lower().strip()

                    line_text = line_text.translate(
                        str.maketrans('', '', pun))

                    m = regex.match(ptn,line_text)
                    if m:
                        df.loc[(df["page_num"] == page) & 
                               (df["tableLineNo"] >= line_no) &
                               (df["line_row"] > 0),
                               "line_row"] = 0
                        print("Page Number Pattern Match",
                              ptn,
                              line_text,
                              m)
                        brk = True
            ...
            for word_weights in keys:
                ...
                for word_weight in word_weights.keys():
                    if word_weight.lower() != "exclude":
                        words = word_weights[word_weight]
                        #Change for using RapidFuzz instead of FuzzyWuzzy
                        #Oct-20-2022 - Changed fuzzy match score to token set ratio
                        part = rp_fz.extractOne(txt,
                                                words,
                                                scorer = rp_fuzz.WRatio)
                        # part = rp_fz.extractOne(txt,
                        ...
                        #                         scorer = rp_fuzz.token_set_ratio)
                        #Oct-20-2022 - Changed fuzzy match score to token set ratio
                        # if (("of supply" in txt.lower()) and ("value" not in txt.lower())) or (("bill" in txt.lower()) and ("state" not in txt.lower())):
                        #     if ("bill" in words) or ("supply") in words:
                        ...
                        # if (("of supply" in txt.lower()) and ("value" not in txt.lower())) or (("bill" in txt.lower()) and ("state" not in txt.lower())):
                        #     if ("bill" in words) or ("supply") in words:
                        #         print(score,part_score,word_weight)
...
@util.timing
def newLineRowCorrection(df):

    def assignVavluesToDf(col_name,
                          col_vals,
                          df,
                          base_col = "token_id"):
        import numpy as np
    ...
    df["line_row_new"] = df["line_row"]
    df["avgDistBtwLines"] = 0.0
    df["avgDistBtwLinesTenPerc"] = 0.0
    df["distBtwAvgAndAbs"] = 0.0
    df_copy = df.copy(deep = True)
    try:
        df_filt_grp = df[df["line_row"]>0].groupby(["page_num"
                                                    ]).agg(
                                                        {"distBtwLine":["mean"]}).reset_index()
        df_filt_grp = df[df["line_row"]>0].groupby(["page_num"
                                                    ]).agg(
                                                        {"distBtwLine":["median"]}).reset_index()
        df_filt_grp.columns = ["page_num",
                               "avgDistBtwLines"]
        for row in df_filt_grp.itertuples():
            df.loc[(df["page_num"] == row.page_num) & 
                   (df["line_row"] > 0),
                   "avgDistBtwLines"] = row.avgDistBtwLines
            df.loc[(df["page_num"] == row.page_num) & 
                   (df["line_row"] > 0),
                   "avgDistBtwLinesTenPerc"] = row.avgDistBtwLines * 0.3

        df["distBtwAvgAndAbs"] = df["distBtwLine"] - df["avgDistBtwLines"]

        df_filt = df[df["line_row"] > 0]
        df_filt = df_filt.sort_values(["page_num",
                                       "line_row",
                                       "tableLineNo"])
        df_grp = df_filt.groupby(["page_num",
                                  "tableLineNo"]).agg({"distBtwLine":["max"],
                                                       "lineWithRightmostItem":["max"],
                                                       "line_row":["max"],
                                                       "avgDistBtwLines":["max"],
                                                       "avgDistBtwLinesTenPerc":["max"],
                                                       "distBtwAvgAndAbs":["max"]}).reset_index()
        df_grp.columns = ['page_num',
                          'tableLineNo',
                          'distBtwLine',
                          'lineWithRightmostItem',
                          'line_row',
                          'avgDistBtwLines',
                          'avgDistBtwLinesTenPerc',
                          'distBtwAvgAndAbs']

        upd_dict = {}
        new_line_row = 1
        prev_line_right_most = 0
        line_right_most_item = 0
        line_row = 0
        prev_line_row = 0
        page_num = 0
        prev_page_num = 0
        ...
        tableLineNo = 0
        avgDistBtwLines = 0
        avgDistBtwLinesTenPerc = 0
        distBtwAvgAndAbs = 0
        for row_ind,row in enumerate(df_grp.itertuples()):
            if row_ind == 0:
                line_right_most_item = row.lineWithRightmostItem
                prev_line_right_most = line_right_most_item
                line_row = row.line_row
                prev_line_row = line_row
                page_num = row.page_num
                prev_page_num = page_num
                tableLineNo = row.tableLineNo
                tkn_ids = list(df[(df["page_num"] == page_num) &
                                  (df["tableLineNo"] == tableLineNo)]["token_id"])
                temp_dict = {tkn_id:new_line_row for tkn_id in tkn_ids}
                upd_dict.update(temp_dict)
            else:
                # print("Inside the else part")
                distBtwLine = row.distBtwLine
                avgDistBtwLines = row.avgDistBtwLines
                line_right_most_item = row.lineWithRightmostItem
                page_num = row.page_num
                tableLineNo = row.tableLineNo
                avgDistBtwLinesTenPerc = row.avgDistBtwLinesTenPerc
                distBtwAvgAndAbs = row.distBtwAvgAndAbs
                # print("Dist less",distBtwLine,
                #       avgDistBtwLines)
                # print("LineWithRightMostItem",
                #       line_right_most_item,
                #       tableLineNo,
                #       prev_line_right_most,
                #       line_right_most_item)

                #Sep 29 2022, changed to accomodate some outlier cases where the diff btw average dist is gt
                #if distBtwLine <= avgDistBtwLines:
                if distBtwAvgAndAbs <= avgDistBtwLinesTenPerc:
                #Sep 29 2022, changed to accomodate some outlier cases where the diff btw average dist is gt
                    # print("Inside the dist less part",
                    #       line_right_most_item,
                    #       prev_line_right_most)
                    if line_right_most_item == 1:
                        if prev_line_right_most == 1:
                            new_line_row += 1
                        prev_line_right_most = line_right_most_item
                    tkn_ids = list(df[(df["page_num"] == page_num) &
                                      (df["tableLineNo"] == tableLineNo)]["token_id"])
                    temp_dict = {tkn_id:new_line_row for tkn_id in tkn_ids}
                    # print("Inside the dist less part: tokens",
                    #       temp_dict,
                    #       new_line_row)
                    upd_dict.update(temp_dict)
                else:
                    print("Inside the dist more part",
                          distBtwAvgAndAbs,
                          avgDistBtwLinesTenPerc)
                    prev_line_right_most = 0
                    if line_right_most_item == 1:
                    ...
                    tkn_ids = list(df[(df["page_num"] == page_num) &
                                      (df["tableLineNo"] == tableLineNo)]["token_id"])
                    temp_dict = {tkn_id:new_line_row for tkn_id in tkn_ids}
                    # print("Inside the dist more part: tokens",
                    #       temp_dict,
                    #       new_line_row)
                    upd_dict.update(temp_dict)
            df = assignVavluesToDf("line_row_new",
                                   upd_dict,
                                   df)
        return df
    except:
...
def addLineItemFeatures_New(DF):
    ...
    try:
        ...
        DF["line_item"] = 0
        DF["line_row"] = 0
        # 23-Sep-2022 - Added new features to solve top or bottom aligned tables - Epson demo
        # DF["line_row_top_align"] = 0
        # DF["line_row_bottom_align"] = 0
        # DF["line_row_center_align"] = 0
        DF["lineWithRightmostItem"] = 0
        # 23-Sep-2022 - Added new features to solve top or bottom aligned tables - Epson demo
        DF["amount_in_line"] = 0
        DF[LI_FIELDS] = 0
        DF["tbl_col_hdr"] = ""
        ...
        #MAY 05 2023 - Don't continue with line items when we encounter GST summary table in next page
        max_scores["diff_max"] = max_scores["score"].max() - max_scores["score"]
        # max_scores["std"] = max_scores["diff_max"].std()
        max_scores["factor"] = max_scores["diff_max"]/max_scores["score"]
        print("Max scores:", max_scores)
        max_scores = max_scores[max_scores["factor"] <= 0.2]
        #MAY 05 2023 - Don't continue with line items when we encounter GST summary table in next page

        pages = list(max_scores["page_num"].unique())
        #21-Sep-2022 - take only pages in a series or single page
        print("Max scores:", max_scores,pages)
        new_pages = []
        if len(pages) > 0:
            pg_num = -1
            scr = -1
            for page_index,page_num in enumerate(pages):
                if page_index == 0:
                    pg_num = page_num
                    new_pages.append(pg_num)
                else:
                    if page_num - pg_num == 1:
                        pg_num = page_num
                        new_pages.append(page_num)
                    else:
                        break
        #21-Sep-2022 - take only pages in a series or single page
        print("Pages:",pages,all_pages,new_pages)

        page_cnt = 0
        #Mar-16-2023 - START - Change done to avoid continuing with line rows for 'n+1'th and onwards when footer is found in 'n'th page
        footer_found = False
        #Mar-16-2023 - END - Change done to avoid continuing with line rows for 'n+1'th and onwards when footer is found in 'n'th page

        for max_row_ind,max_row in max_scores.iterrows():
            #Mar-16-2023 - START - Change done to avoid continuing with line rows for 'n+1'th and onwards when footer is found in 'n'th page
            if footer_found:
                #May-05-2023 - START - Do not break when there are more than one page with header starting in each page
                # break
                print("Footer found at",max_row["page_num"])
                pg_num = max_row["page_num"]
                if pg_num > max(new_pages):
                    break
            elif max_row["page_num"] not in new_pages:
                break
                #May-05-2023 - START - Do not break when there are more than one page with header starting in each page
            #Mar-16-2023 - END - Change done to avoid continuing with line rows for 'n+1'th and onwards when footer is found in 'n'th page
            #Update header line
            ...
            print("Min page:",
                  ...
                  hdr_val)
            # lines = list(DF_hdr[(DF_hdr["page_num"] >= min_page) &
            #                     (DF_hdr["page_num"] <= min_page)]["tableLineNo"])
            #Changes made on Apr 06, 2022
            lines = sorted(lines)
                    ...
                    while scr > 0:
                        ...
                        if len(prev_lines) > 0  and (rowcount < len(prev_lines)):
                            ...
                            if line_top - line_down < .05:
                                ...
                                print("Is header previous line",
                                      check_hdr(DF,prev_line),
                                      prev_line,
                                      scr)
                                if scr >= 200 and scr_1 > 0:
                                    if check_hdr(DF,prev_line):
            ...
            print("Potential lines before loop",pot_lines)
            ...
            if len(pot_lines) > 0:
                ...
                footer_lineno = -1
                for line_no in pot_lines:
                    # print("Line Number:",line_no,pot_lines)
                    if found:
                        break
                    DF_line = DF[DF['tableLineNo'] == line_no]
                    #13-Dec-2022 - Skip the loop if no content is present
                    DF_line = DF_line[DF_line["is_nothing"] == False]
                    if DF_line.shape[0] == 0:
                        continue
                    #13-Dec-2022 - Skip the loop if no content is present
                    for i,row in DF_line.iterrows():
                        line_text = str(row["line_text"]).lower().strip()
                        #Mar-19-2023 - Find Page # of # as a footer pattern
                        # l_line_text = line_text
                        #Mar-19-2023 - Find Page # of # as a footer pattern
                        line_text = line_text.translate(str.maketrans('', '', punc))
                        #Jan-18-2023 - Adjust score cut-off by length of string
                        import math
                        new_cutoff = 100.0
                        if len(line_text) >= 4:
                            m = 2 * len(line_text) - 1
                            new_cutoff = math.floor((1 - 1/m) * 100)/100.0
                        adj_cutoff = min(new_cutoff * 100.00,90.00)
                        print("Footer Line Text",
                              line_text,
                              adj_cutoff,
                              new_cutoff)
                        # matches = rp_fz.extractOne(line_text,
                        ...
                        matches = rp_fz.extractOne(line_text,
                                                   footer_keywords,
                                                   scorer = rp_fuzz.ratio,
                                                   score_cutoff = adj_cutoff)
                        # matches1 = rp_fz.extractOne(line_text,
                        #                            footer_keywords,
                        #                            scorer = rp_fuzz.ratio,
                        #                            score_cutoff = 1.00)
                        #Mar-19-2023 - Add Page # of # as a pattern to recognize footer
                        # if not matches:
                        #     pun = string.punctuation
                        #     l_line_text = l_line_text.translate(
                        #         str.maketrans('', '', pun))
                        #     import regex
                        #     ptn = 'page[ ]{0,1}[0-9]{1,3}[ ]{0,1}of[ ]{0,1}[0-9]{1,3}'
                        #     m = regex.match(ptn,l_line_text)
                        #     if not m:
                        #         matches = m
                        #         adj_cutoff = 0
                        ...
                        #Mar-19-2023 - Add Page # of # as a pattern to recognize footer
                        #Jan-18-2023 - Adjust score cut-off by length of string
                        # print(matches1,line_text,adj_cutoff)
                        print("Matched",
                              matches,
                              line_text,
                              adj_cutoff,
                ...
                if len(line_noanchor) > 0:
                    ...
                    #Mar-16-2023 - START - Change done to avoid continuing
                    #with line rows for 'n+1'th and onwards when footer is
                    #found in 'n'th page
                    noanchor = pot_lines[:-2:-1][0]
                    print("Footer Line No:",footer_lineno)
                    # if footer_lineno == -1:
                    #     noanchor = pot_lines[:-2:-1][0]
                    # else:
                    #     noanchor = -1
                ...
                #found in 'n'th page
                pot_reduced_lines = pot_lines[:pot_lines.index(noanchor)+1]
                print("No Anchor",noanchor)
                # pot_reduced_lines = None
                # if noanchor == -1:
                #     print("No Anchor",noanchor)
                #     if pot_reduced_lines is not None:
                #         break
                #     else:
                #         pot_reduced_lines = pot_lines[:pot_lines.index(noanchor)+1]
                # else:
                ...
                for m in rowparts:
                    ...
                    diffs = [sum_pattern[j] - sum_pattern[j+1]
                             for j in range(len(sum_pattern)-1)]
                    print("Sum pattern:",sum_pattern,diffs)
                    sum_diffs = 0
                    for i in range(len(diffs)):
                        if abs(diffs[i]) <= 1:
                            sum_diffs += diffs[i]
                ...
                pot_reduced_lines = pot_lines[:pot_lines.index(noanchor)+1]
                rowparts = list(range(min(len(pot_reduced_lines),10),0,-1))
                print("potential reduced lines 2:",pot_reduced_lines)
                coords = []
                for line_ind, line_no in enumerate(pot_reduced_lines):
                    DF_line = DF[DF['tableLineNo'] == line_no]
                    line_top = max(list(DF_line["line_top"]))
                ...
                pot_reduced_lines = pot_lines[:pot_lines.index(noanchor)+1]
                line_valign.append(pot_reduced_lines[0])
                print("potential reduced lines 3:",pot_reduced_lines)
                print("potential line dists:",line_dist,line_valign)
                hdrs = DF[DF["tableLineNo"] == pot_reduced_lines[0]][["line_down",
                                                                      "line_left",
                                                                      "line_right",
                                                                      "line_text"]
                ...
                print("potential reduced lines 3b:",pot_reduced_lines)
                for line_ind in range(1,len(pot_reduced_lines)):
                    line = DF[DF["tableLineNo"] == pot_reduced_lines[line_ind]][
                        ["line_top","line_left",
                         "line_right","line_text"]].drop_duplicates(
                    ...
                    dist = line_top - line_down
                    mtch = 0
                    # print("Line check:",pot_reduced_lines[line_ind],mtch)
                    if dist < .1:
                        for j in zip(first_lefts,first_rights):
                            for k in zip(lefts,rights):
                                if (k[0] > j[1]) or (k[1] < j[0]):
                        ...
                        mtch = 0
                        #May 30 2023 - Don't stop line item numbering when distance between two lines
                        #isn't less than .1 because when line items continue to the
                        #next page w/o headers, the distance will be high                        
                        # break
                        #May 30 2023 - Don't stop line item numbering when distance between two lines
                        #isn't less than .1 because when line items continue to the
                        #next page w/o headers, the distance will be high                        
                    ...
                    if mtch == 0:
                        ...
                        first_rights = list(hdrs["line_right"].values())
                        line_down = max(list(hdrs["line_down"].values()))
                        #May 30 2023 - Increase the number of lines that can be
                        #skipped before deciding there aren't any line items
                        #This happens for multipage invoice with no headers but some
                        #free text are in between line rows of one page to another 
                    # if lines_notfound > 3:
                    if lines_notfound > 10:
                        #May 30 2023 - Don't stop line item numbering when distance between two lines
                        #isn't less than .1 because when line items continue to the
                        #next page w/o headers, the distance will be high                        
                        line_down = 0
                ...
                print("potential lines:",line_valign)
                #Update row breaks
                # t = time.time()
                max_lefts = []
                max_rights = []
                max_alls = []
                #Sep-29-2022 Find max left and right tokens
                max_left_tokens = []
                max_right_tokens = []
                #Sep-29-2022 Find max left and right tokens
                line_dist_align = list(set(line_dist).union(set(line_valign)))
                line_dist_align = sorted(line_dist_align)
                print("Line Dist Align:",
                      line_dist_align,
                      max_lefts,
                      line_dist,
                      line_valign)
                for line_index,line_no in enumerate(line_dist_align):
                    if line_index == 0:
                        #21-Sep-2022 - don't consider tokens that does not have content
                        # neighbours = list(DF[(DF["tableLineNo"] == line_no)])
                        print("Check for potential row break")
                        neighbours = list(DF[(DF["tableLineNo"] == line_no)])
                        # print("Neighbours ccccsaasdasd",neighbours)
                        #21-Sep-2022 - don't consider tokens that does not have content
                        if len(neighbours) == 1:
                            continue
                    #21-Sep-2022 - don't consider tokens that does not have content
                    # lefts = list(DF[DF["tableLineNo"] == line_no]["line_left"])
                    lefts = list(DF[(DF["tableLineNo"] == line_no) &
                                    (DF["is_nothing"] == False)]["line_left"])
                    # print("Lefts sdsdsadasdsafsf",lefts)
                    #21-Sep-2022 - don't consider tokens that does not have content

                    #21-Sep-2022 - don't consider tokens that does not have content
                    if len(lefts) < 1:
                        continue
                    print("Lefts present:",lefts)
                    #21-Sep-2022 - don't consider tokens that does not have content
                    ...
                    #21-Sep-2022 - don't consider tokens that does not have content
                    # rights = list(DF[DF["tableLineNo"] == line_no]["line_right"])
                    rights = list(DF[(DF["tableLineNo"] == line_no) &
                                     (DF["is_nothing"] == False)]["line_right"])
                    if len(rights) < 1:
                        continue
                    print("Rights present:",rights)
                    #21-Sep-2022 - don't consider tokens that does not have content
                    max_right = rights[max_ind]
                    max_rights.append(max_right)

                    #29-Sep-2022 - Find Max left and right by token
                    left_tokens = list(DF[(DF["tableLineNo"] == line_no) &
                                    (DF["is_nothing"] == False)]["left"])
                    right_tokens = list(DF[(DF["tableLineNo"] == line_no) &
                                     (DF["is_nothing"] == False)]["right"])
                    max_right_token_ = max(right_tokens)
                    max_right_tokens.append(max_right_token_)
                    max_ind_token = right_tokens.index(max_right_token_)
                    max_left_token_ = left_tokens[max_ind_token]
                    max_left_tokens.append(max_left_token_)
                    #29-Sep-2022 - Find Max left and right by token
                    ...
                    max_alls.append((max_left,
                                     ...
                                     line_no))
                    #29-Sep-2022 - Find Max left and right by token
                    # print("Maximum All:",max_alls,max_lefts,line_no,page_num)
                    # print("Potential Reduced Lines 4:",pot_reduced_lines)
                    # print("Leng of max lefts",len(max_lefts))

                if len(max_lefts) > 0:
                    #May 31 2023 - Take the dimensions of extreme left & right to
                    max_left = max(max_lefts)
                    min_left = min(max_lefts)
                    #May 31 2023 - Take the dimensions of extreme left & right to
                    max_ind = max_lefts.index(max_left)
                    max_all = max_alls[max_ind]
                    #May 31 2023 - Take the dimensions of extreme left & right to
                    ...
                    # print("Max",max_alls)
                    #29-Sep-2022 - Find Max left and right by token
                    max_left_token = max(max_left_tokens)
                    # max_right_token = max(max_right_tokens)
                    #29-Sep-2022 - Find Max left and right by token
                    ...
                    for all_ind,all_ in enumerate(max_alls):
                        ...
                        #May 31 2023 - Take the dimensions of extreme left & right to
                            ...
                            if row_first:
                                DF.loc[(DF["tableLineNo"] == line_no),
                                       "line_row"] = row_number
                            #27-Sep-2022 - Add an indicator that this line contains extreme right info
                            # print("Table Line No",
                            #       line_no,
                            #       right,
                            #       max_left_token,
                            #       all_)
                            #29-Sep-2022 - Find Max left and right by token
                            if right > max_left_token:
                                # print("Table Line No",line_no,"Updated")
                                # print("filtered",
                                #       DF[DF["tableLineNo"] == line_no].shape)
                                DF.loc[(DF["tableLineNo"] == line_no),
                                       "lineWithRightmostItem"] = 1
                                # DF.to_csv(r"d:\ftrs_temp.csv",index = False)
                            #27-Sep-2022 - Add an indicator that this line contains extreme right info
                    ...
                    # t = time.time()
                    pot_reduced_lines = pot_lines[:pot_lines.index(noanchor)+1]
                    print("pot reduced lines 5:",pot_reduced_lines)
                    line_nums = []
                    ...
                    for line_no in pot_reduced_lines:
                        ...
                        DF.loc[(DF["tableLineNo"] == line_no)
                               ,["line_amount","amount_in_line"]] = [int(count > 0),
                                                                     count]
                        if count > 0:
                            line_nums.append(line_no)
                    ...
                    line_dist_valign = list(set(line_dist).union(set(line_valign)))
                    line_dist_valign = sorted(line_dist_valign)
                    print("line dist valign 2",line_dist_valign)
                    for line_no in line_dist_valign:
                        #Working code before 15th June 2021
                        line = DF[DF["tableLineNo"] == line_no][
                            ["text","left","right","top"]].drop_duplicates(
...
def extract_text_features(df):
    ...
    def extract_token_specific_features(df):
        ...
        for index, row in df.iterrows():
            text = row['text']
            try:
                #Sep-29-2022 Calculate is_date only when num of characters gt 6
                # pd.to_datetime(text)
                # is_date.append(1)
                if len(text) >= 6:
                    pd.to_datetime(text)
        ...
        for line_text,line_top,line_left in lines:
            ...
            if util.isTokenAmount(text):
                ...
                df.loc[(df["line_text"] == line_text) &
                        ...
                        "text","line_text"]] = pd.Series(l).values
...
#     #May 31 2023 - Remove outlier boxes for updating this info - Start
#     df_ht_desc = df.groupby('page_num')['height'].describe().reset_index()
#     df_ht_desc.columns = ['page_num', 'ht_count', 'ht_mean', 'ht_std',
#                           'ht_min', 'ht_1_qrt', 'ht_2_qrt', 'ht_3_qrt',
#                           'ht_max']
#     df_ht_desc = df_ht_desc.drop((['ht_count','ht_mean','ht_std','ht_min',
#                                    'ht_max','ht_2_qrt']), axis = 1)
#     df = pd.merge(df,df_ht_desc,on="page_num")
#     df["outlier_cond_big"] = ((df["ht_3_qrt"] - df["ht_1_qrt"]) * 1.5) + df["ht_3_qrt"]
#     #May 31 2023 - Remove outlier boxes for updating this info - End
...
#23-Sep-2022 - Add distance between each lines
# @util.timing
def findDistBetweenLines(DF):
    ...
    DF_copy = DF.copy(deep = True)
    try:
        DF.sort_values(["page_num","tableLineNo"])
        DF_grp = DF.groupby(["page_num",
                             "tableLineNo"]).agg({"line_top":["mean"],
                                                  "line_down":["mean"]}).reset_index()
        DF_grp.columns = ['page_num',
                          'tableLineNo',
                          'avgLineTop',
                          'avgLineDown']
        DF_grp = DF_grp.sort_values(['page_num',
                                     'tableLineNo'])

        DF_grp["line_down_lead"] = DF_grp["avgLineDown"].shift(periods = 1,
                                                               fill_value = 0)
        DF_grp["distBtwLine"] = DF_grp["avgLineTop"] - DF_grp["line_down_lead"]

        df_stLine = DF_grp.groupby(["page_num"]).agg({"tableLineNo":["min"]}).reset_index()
        df_stLine.columns = ["page_num","tableLineNo"]
        for row in df_stLine.itertuples():
            DF_grp.loc[(DF_grp["page_num"] == row.page_num) &
                       (DF_grp["tableLineNo"] == row.tableLineNo),
                       "distBtwLine"] = 0.0
        ...
        # rem_cols = [col for col in cols if "_x" in col]
        # ren_cols = [col for col in cols if "_y" in col]
        rem_cols = ["avgLineTop_x",
                    "avgLineDown_x",
                    "distBtwLine_x"]
        ren_cols = ["avgLineTop_y",
                    "avgLineDown_y",
                    "distBtwLine_y"]
        DF = DF.drop(rem_cols,
                     axis = 1)
        DF = DF.drop('line_down_lead',
                     axis = 1)
        ren_dict = {col:col.replace("_y","") for col in ren_cols}
        DF = DF.rename(columns = ren_dict)
        DF["distBtwLine"] = DF["distBtwLine"].fillna(0)

        return DF
    except:
        print("findDistBetweenLines",
              traceback.print_exc())
        return DF_copy
...
def read_lines_from_table_new(DF):
    ...
    # DF.to_csv(r"d:\ftrs.csv",index = False)
    ...
    df_ht_desc = df_ht_desc.drop((['ht_count','ht_mean','ht_std','ht_min',
                                   'ht_max']), axis = 1)
    DF = pd.merge(DF,df_ht_desc,on="page_num")
    #DF["outlier_cond_big"] = ((DF["ht_3_qrt"] - DF["ht_1_qrt"]) * 5) + DF["ht_3_qrt"]
    #May 31 2023 - Remove outlier boxes for updating this info - End
    DF["outlier_cond_big"] = ((DF["ht_2_qrt"]) * 5)
    #jun 13 2023 -added to eliminate zero in tablelineno
    ...
    df = DF[cols]
    #May 31 2023 - Remove outlier boxes for updating this info - End
    df = df[(df["height"] < df["outlier_cond_big"])]
    #May 31 2023 - Remove outlier boxes for updating this info - End
...
def potential_itemValue(df):
    ...
    df['is_prob_unitPrice'] = 0
    df_copy = df.copy(deep = True)
    #21-Sep-2022 Use this flag to run the same logic again without extracted_amount >= 0.0
    found = False
    #21-Sep-2022 Use this flag to run the same logic again without extracted_amount >= 0.0
    try:
        ...
        for page in df['page_num'].unique():
            ...
            temp_df = df[(df['page_num'] == page) &
                         (df['line_row'] > 0)]
            print("temp_df length",temp_df.shape[0])
            if temp_df.shape[0] > 0:
                for line in temp_df['line_row'].unique():
                    ...
                    # Epson demo trial 1
                    qtys = line_df[(line_df["is_qty1"] >= 0.85)]
                    # print("Quantity df 85 ",qtys.shape)
                    if qtys.shape[0] == 0:
                        qtys = line_df[(line_df["is_qty"] >= 0.2) & 
                                       (line_df["extracted_amount"] > 0.0)]
                    # qtys = qtys.sort_values(["is_qty1"],
                    #                         ascending = [False])
                    # print("qtys",qtys)
                    # Epson demo trial 1
                    ...
                    if qtys.shape[0] == 0:
                        qtys = line_df[(line_df["is_qty"] == 0.0) &
                                       (line_df["ngbr_qty"] == 1)]
                    #21-Sep-2022 Epson demo trial 1
                    # else:
                    ...
                    # print("qty vals",qty_vals)

                    # print("Quantities:",qty_vals)
                    # Epson demo trial 1
                    rates = line_df[(line_df["is_unit_price1"] >= 0.85) & 
                                    (line_df["extracted_amount"] > 0.0)]

                    if rates.shape[0] == 0:
                        rates = line_df[(line_df["is_unit_price"] >= 0.2) & 
                                        (line_df["extracted_amount"] > 0.0)]
                    # print("rates:",rates)
                    # Epson demo trial 1
                    rates = rates.sort_values(["is_unit_price1"],
                                            ascending = [False])
                    ...
                    vals = line_df[(line_df["is_item_val1"] >= 0.85) & 
                                   (line_df["extracted_amount"] > 0.0)]

                    if vals.shape[0] == 0:
                        vals = line_df[(line_df["is_item_val"] >= 0.2) & 
                                       (line_df["extracted_amount"] > 0.0)]

                    vals = vals.sort_values(["is_item_val1"],
                                            ascending = [False])

                    # print("Item Value:",vals)
                    # Epson demo trial 1
                    if vals.shape[0] == 0:
        ...
        #22-Sep-2022 - if the logic doesn't work with extracted amount > 0, try without that
        if not found:
            print("Probable Item Value started")
            for page in df['page_num'].unique():
                # print("Loop page_num:", page)
    ...
    try:
        ...
        #May 13 2022 - using itertuples instead of iterrows
            ...
            for text_ind,text in enumerate(texts):
                ...
                #Oct-20-2022 - translate short keywords to actual words
                mod_text = " ".join(["number" if m == "no" else m for m in mod_text.split(" ")])
                mod_text = " ".join(["invoice" if m == "inv" else m for m in mod_text.split(" ")])
                ...
                if text_to_compare == "":
                    if text_ind < len(texts) - 1:
                        ...
                        break
                if len_words > 0:
                    #26-Sep-2022 the words are concatenated. So, the len_words need not be appended
                ...
                for key in hdr_keys_vals:
                    ...
                    if isinstance(vals[0],str):
                        match = fz_match_hdrTxt(text_to_compare,
                                                processed_text)
                    else:
                        match = fz_match_hdrTxt(text_to_compare,
                                                vals)
...
#     #23-Sep-2022 find simple distance between lines
#     # df = findDistBetweenLines(df)
#     # df.to_csv(r"d:\dddd.csv",index = False)
#     #23-Sep-2022 find simple distance between lines
...
def process_df_features(df):
    ...
    try:
        ...
        # df.to_csv(r"d:\aaaa.csv",index = False)
        #23-Sep-2022 find simple distance between lines
        print("Before Dist Between Lines")
        df = findDistBetweenLines(df)
        print("After Dist Between Lines")
        ...
        #It also extracts nearby words and their token specific features
        df = extract_text_features(df)
        # df.to_csv(r"d:\bbbb.csv",index = False)
        ...
        #df = extract_max_amount_features(df)
        #findRegions of text
        #17-Aug-2023 - Default region to 0
        # df = findRegions(df)
        df["region"] = 0
        #17-Aug-2023 - Default region to 0
        return df
...
def process_df_lines(df):
    ...
    try:
        ...
        #Added code to correct line_row assignent if present before header row
        df = correctLineRows(df)
        #Sep-28-2022 - Adjust line row based on distance between different lines
        df = newLineRowCorrection(df)
        #May 28, 2022 - Added code to adjust description if their column header is not overlapping
        df = correctLineRowsFzScore(df)
...
def extract_features(docInput):
    ...
    try:
        ...
        #Jun 23 2023 - Don't use multiprocessing
        t = time.time()
        DFs = []
        print("Page length",
              len(pages))
        ...
        #21-Sep-2022 Make Unique token ID across all pages in a document
        # DF_All = makeTokenIdsUnq(DF_All)
        DF_All.insert(0,
                      "new_token_id",
                      range(0,0 + len(DF_All)))
        DF_All["token_id"] = DF_All["new_token_id"]
        ...
        print("Dataframe Index:",
              DF_All.index)
        #21-Sep-2022 Make Unique token ID across all pages in a document
        #May 06, 2022 - These functions should be applied on the entire dataframe and not page wise
        # DF_All = extract_max_amount_features(DF_All)
        DF_All = extract_max_amount_features_new(DF_All)
        if os.path.exists("d:"):
            DF_All.to_csv("d:/ftrs_0.csv",
                          index = False)
        ...
        print("Is line_row present",
              "line_row" in DF_All.columns.values)
...
if __name__ == "__main__":
    docid = "7g5PeKqzEK-UTWwJeDF5C-OWYVk4tKH3VWTE-0"
    container = "e6ea6f6c-9dca-4fac-a76a-a98af245132d"
    inp = {
           "documentId":docid,
           "client_folder":"",
           "container":container,
           "page_details":
               [
                   {"image_path":container+"/"+docid+"_0-pre.tiff",
                    "ocr_path":container+"/"+docid+"_0-pre.tiff.ocr.json",
